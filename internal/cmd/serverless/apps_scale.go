...
func bindScaleFlags(cmd *cobra.Command, flags *scaleFlags) {
	...
	f.Int32Var(&flags.gpusPerWorker, "gpus-per-worker", 0, "GPUs allocated per worker ("+gpusPerWorkerValuesText()+")")
...
func workerConfigPatchFromFlags(cmd *cobra.Command, flags scaleFlags) (*serverlessapi.WorkerConfigPatch, error) {
	if err := validateGPUsPerWorkerFlag(cmd, flags.gpusPerWorker); err != nil {
		return nil, err
	}
